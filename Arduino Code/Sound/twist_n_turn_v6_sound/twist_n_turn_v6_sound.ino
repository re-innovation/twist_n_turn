...
long int start_time;
int i = 0;
int SOUND_SENSITIVITY = 100;    // Store the trigger sensitivity value - read on A3.
...
void t1SCallback() {
  SOUND_SENSITIVITY = analogRead(3);
  SOUND_SENSITIVITY = map(SOUND_SENSITIVITY, 0, 1023, SOUND_MIN, SOUND_MAX);
  SOUND_SENSITIVITY = constrain(SOUND_SENSITIVITY, SOUND_MIN, SOUND_MAX);
  if (DEBUG == true)
  {
    Serial.print("SENSE:");
  ...
  if (!t1S.isFirstIteration())
  {
    ...
    {
      if ((sound_max - (sound_average + SOUND_SENSITIVITY)) < LED_SPEED_KNEE)
      {
        led_speed = map((sound_max - (sound_average + SOUND_SENSITIVITY)), 5, LED_SPEED_KNEE, LED_SPEED_MAX_1, LED_SPEED_MIN_1);
        led_speed = constrain(led_speed, LED_SPEED_MIN_1, LED_SPEED_MAX_1);
      }
      else
      {
        led_speed = map((sound_max - (sound_average + SOUND_SENSITIVITY)), LED_SPEED_KNEE, 1023, LED_SPEED_MAX_2, LED_SPEED_MIN_2);
        led_speed = constrain(led_speed, LED_SPEED_MIN_2, LED_SPEED_MAX_2);
      }
      Serial.print("TRIG:");
...
void loop()
{
  ...
  if (led_trigger_flag == true)
  {
    ...
    {
      start_time = millis() + ((led_speed * 10) / 255);
      for (int x = 0; x < strip.numPixels(); x++)
      { // For each pixel in strip...

        //        // This code sets the colour flick through colours
        //        strip.setPixelColor(x, *RainbowColours[i]);

        // This code sets the colour fade through the rainbow colours
        strip.setPixelColor(x, Wheel(i));
