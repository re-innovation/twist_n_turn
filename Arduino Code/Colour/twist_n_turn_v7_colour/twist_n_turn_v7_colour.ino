...
int COLOUR_TRIGGER_LEVEL = 200; // Holder for trigger sensitivity value - read on A3
...
void t1SCallback() {
  COLOUR_TRIGGER_LEVEL = analogRead(3);
  COLOUR_TRIGGER_LEVEL = map(COLOUR_TRIGGER_LEVEL, 0, 1023, WHITE_MIN, WHITE_MAX);
  COLOUR_TRIGGER_LEVEL = constrain(COLOUR_TRIGGER_LEVEL, WHITE_MIN, WHITE_MAX);
  if (DEBUG == true)
  {
    Serial.print("SENSE:");
    Serial.println(COLOUR_TRIGGER_LEVEL);
  }
  if (!t1S.isFirstIteration())
  {
    ...
    {
      // This stretches the values so they are futher apart (simplifies colours more)
      if (red_t > green_t)
      {
        if (green_t > SENSITIVITY_ADJUST)
        {
          green_t = green_t - SENSITIVITY_ADJUST;
        }
        else
        {
          green_t = 0;
      ...
      if (red_t > blue_t)
      {
        if (blue_t > SENSITIVITY_ADJUST)
        {
          blue_t = blue_t - SENSITIVITY_ADJUST;
        }
        else
        {
          blue_t = 0;
      ...
      if (green_t > red_t)
      {
        if (red_t > SENSITIVITY_ADJUST)
        {
          red_t = red_t - SENSITIVITY_ADJUST;
        }
        else
        {
          red_t = 0;
      ...
      if (DEBUG == true)
      {
        Serial.print("Rn:\t"); Serial.print(red_t);
        Serial.print("\tGn:\t"); Serial.print(green_t);
        Serial.print("\tBn:\t"); Serial.print(blue_t);
        Serial.println("\t");
      }
      //  This adds weight to the values
